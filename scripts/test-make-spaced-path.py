...
CHILD_MARKER = "HOME_IBEACON_MAKE_SPACE_CHILD"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="home-ibeacon-make-space-") as temporary:
        root = Path(temporary)
        ...
        subprocess.run(["git", "-C", copied, "add", "-f", "-N", "--", *tracked], check=True)
        environment = os.environ.copy()
        environment[CHILD_MARKER] = "1"
        make = environment.get("HOME_IBEACON_MAKE", "make")
        repository_makefile = str(copied / "Makefile")
        subprocess.run([make, "-f", repository_makefile, "check"], cwd=caller, env=environment, check=True, timeout=180)
